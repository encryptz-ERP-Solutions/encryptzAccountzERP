...
## 🔐 Authentication System Migration (NEW)

### 4. Refresh Tokens Table
**File**: `sql/2025_11_20_create_refresh_tokens_table.sql`

**Purpose**: Creates the refresh_tokens table for JWT authentication with token rotation support.

**Features**:
- Stores hashed refresh tokens (SHA256)
- Token rotation tracking via `replaced_by_token_id`
- IP address tracking for security audit
- Automatic cleanup function for expired tokens
- Indexes for fast token lookup and user queries
...
WHERE table_schema = 'core' AND table_name = 'refresh_tokens';
-- Expected: 1

-- Check cleanup function exists
SELECT routine_name FROM information_schema.routines 
WHERE routine_schema = 'core' AND routine_name = 'cleanup_expired_refresh_tokens';
-- Expected: cleanup_expired_refresh_tokens
```

**Documentation**:
- See `docs/auth-design.md` for complete authentication design
- See `docs/auth-implementation-summary.md` for implementation details
...
**Latest Update**: Added authentication system with refresh token rotation
